            ...
            <div className='flex flex-col items-center w-full max-w-full px-4 sm:px-8 mx-auto'>
                {disabled && (
                    <div className="text-center text-sm text-gray-500 mb-4">
                        ⏳ Waiting for {opponent?.name} to {gameState.showOpponentGuess ? "end turn..." : "make a move..."} 
                    </div>
                )}
                <motion.div
                    key={currentPlayer?.id+"_name"}
                ...
                >
                    ...
                    >
                        {disabled? `${currentPlayer?.name}'s Turn` : "Your Turn"}
                    </Badge>
                </motion.div>

                <motion.div
                    key={currentPlayer?.id+"_gameboard"}
                ...
                >
                    {gameMoves.filter(move => move.playerId === currentPlayer?.id)
                        .map((move, index) => (
                            <div key={index} className="flex gap-2 items-center">
                                ...
                                >
                                    {move.result.dead } d, {move.result.injured} inj
                                </Badge>
                            </div>
                        )
                    )}
                    ...
                    {disabled ? (
                        <motion.div
                            ...
                            initial={{ opacity: 1 }}
                            animate={gameState.showOpponentGuess ? { opacity: 1 }: { opacity: [1, 0.2, 1] } }
                            transition={gameState.showOpponentGuess ? {} : { repeat: Infinity , duration: 4, ease: "easeInOut" } }
                        >
                            <span className="text-gray-500 font-semibold text-sm w-15">
                                {gameState.showOpponentGuess ? "Guess:" : `${opponent?.name} is thinking...` }
                            </span> 
                            <InputOTP
                                maxLength={4}
                                pattern={REGEXP_ONLY_DIGITS}
                                value={gameState.showOpponentGuess ? latestOpponentMove?.guess : ""}
                                readOnly
                                disabled
                            >
